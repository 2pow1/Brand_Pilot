...
test('creates a deterministic mock draft', () => {
  ...
  assert.equal(draft.title, '[검토 초안] Small business marketing signals');
  assert.match(draft.body, /GrowthLine/);
  assert.equal(draft.angle, 'problem-awareness');
  assert.equal(draft.cta, '상담 채널 확인하기');
...
test('omits mock draft CTA when CTA is disabled', () => {
  ...
  assert.equal(draft.ctaUrl, '');
  assert.match(draft.body, /GrowthLine은 작은 사업의 브랜딩 메시지/);
});
...
test('normalizes the client draft schema into the app draft shape', async () => {
  const responseDraft = {
    ...
    body: '작은 사업의 브랜딩은 슬로건보다 고객이 반복해서 이해할 수 있는 문장부터 정리해야 합니다.',
    cta: {
      label: '상담 채널 확인하기',
